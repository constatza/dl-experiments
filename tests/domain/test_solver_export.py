...
def solver_result_full():
    ...
    r1 = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float64)
    r2 = torch.tensor([0.5, 0.0, 0.0], dtype=torch.float64)
    x1 = torch.tensor([0.1, 0.0, 0.0], dtype=torch.float64)
    x2 = torch.tensor([0.2, 0.0, 0.0], dtype=torch.float64)
    p1 = torch.tensor([1.0, 0.0, 0.0], dtype=torch.float64)
    p2 = torch.tensor([0.5, 0.0, 0.0], dtype=torch.float64)
...
class TestIterationHistoryExport:
    ...
    def test_full_history_npz(self, tmp_path: Path, solver_result_full):
        """Test NPZ export of full history, including real vector data."""
        ...
        with np.load(npz_file) as data:
            ...
            # Regression guard: torchalg's VectorHistory exposes to_tensor(),
            # not to_array() — exported arrays must contain real vector data,
            # not an object-wrapped VectorHistory.
            np.testing.assert_allclose(data["residuals"][0], [1.0, 0.0, 0.0])
            np.testing.assert_allclose(data["solutions"][1], [0.2, 0.0, 0.0])
            np.testing.assert_allclose(data["directions"][0], [1.0, 0.0, 0.0])
