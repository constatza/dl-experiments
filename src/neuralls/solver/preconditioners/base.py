...
class ContextualPreconditioner(Preconditioner):
    """Base for preconditioners needing iteration context.

    These preconditioners vary behavior based on iteration state.
    Override apply() to accept both residual AND context (required, not optional).

    Mathematical interpretation:
        z_k = M_k^{-1}(r_k) or z_k = f_k(r_k)

    where M_k or f_k may vary with iteration k.
    ...
    @abstractmethod
    def apply(self, residual: NDArray, context: PreconditionerContext | None = None) -> NDArray:
        """Apply preconditioner with iteration context.

        Args:
            residual: Residual vector r_k
            context: Iteration state (required for contextual preconditioners)

        Returns:
            Preconditioned residual z_k

        Raises:
            ValueError: If context is None for contextual preconditioners
        """
        ...

    def needs_context(self) -> bool:
        """Runtime check for context requirement.

        Returns:
            True (contextual preconditioners always need context)
        """
        return True
