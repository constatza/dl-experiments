...
from ..preconditioner_factory import (
    create_all_preconditioners,
    make_neural_preconditioner,
)
...
def compare_preconditioners(
    *,
    ...
    if context is None:
        raise ValueError(
            "Data config is required to locate normalized.npz for comparison."
        )
    ...
    dataset_id = (
        getattr(context.data, "dataset_id", None) if context is not None else "manual"
    )
    ...
    if (
        ...
        and context.test_solutions_path is not None
        and config_rhs is None
        and supplied_rhs is None
    ):
    ...
    for entry in solver_entries:
        ...
        if entry.type not in preconditioners:
            raise ValueError(
                f"Unknown solver/preconditioner type '{entry.type}' requested in solver config."
            )
    ...
    if save_plots:
        try:
            ...
            plot_condition_numbers(
                cond_numbers, save_dir=figures_root, suffix=spectra_suffix
            )
