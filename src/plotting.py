...
def plot_parity_and_residuals(y_true: np.ndarray, y_pred: np.ndarray,
    ...
    ax.plot([y_min - pad, y_max + pad], [y_min - pad, y_max + pad],
            linestyle="dashed", color=set1(0.1), label="y = x")
    ax.set_xlabel("True")
        ...
        if residuals and len(residuals) > 0:
            ...
            if method_name in metadata and metadata[method_name] is not None:
                ...
                if hasattr(meta, 'residual_iters') and hasattr(meta, 'applied_iters'):
                    ...
                    applied_iters = getattr(meta, "applied_iters", None) or residual_iters
                    if residual_iters is not None:
                        label = f"{method_name} (tr={residual_iters}, ap={applied_iters})"
...
def plot_prediction_diagnostics(
    ...
    ax7.bar(x_pos - width/2, pred_vals, width, label='Predictions', alpha=0.8, color='blue')
    ax7.bar(x_pos + width/2, true_vals, width, label='Targets', alpha=0.8, color='orange')
    ...
    ax8.plot([y_min - pad, y_max + pad], [y_min - pad, y_max + pad],
...
{"⚠️  WARNING: Scale ratio > 10x!" if scale_ratio > 10 or scale_ratio < 0.1 else "✓ Scale ratio looks reasonable"}
    ...
    fig.suptitle('Prediction Diagnostics - Scaling Analysis',
